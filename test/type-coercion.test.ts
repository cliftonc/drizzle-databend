...
describe('type coercion', () => {
  ...
  it('should compare integer column with gt()', async () => {
    const result = await db.select().from(products).where(gt(products.quantity, 5));
  ...
  it('should compare integer column with eq()', async () => {
    const result = await db.select().from(products).where(eq(products.quantity, 0));
    expect(result).toHaveLength(1);
    expect(result[0]!.name).toBe('Doohickey');
  });

  it('should compare double column with gt()', async () => {
    const result = await db.select().from(products).where(gt(products.price, 20.0));
    expect(result).toHaveLength(2);
    for (const row of result) {
  ...
  it('should compare double column with lt()', async () => {
    const result = await db.select().from(products).where(lt(products.price, 10.0));
    expect(result).toHaveLength(2);
    for (const row of result) {
  ...
  it('should compare integer column with gte()', async () => {
    const result = await db.select().from(products).where(gte(products.quantity, 10));
    expect(result).toHaveLength(2);
  });

  it('should handle integer 0 comparison', async () => {
    const result = await db.select().from(products).where(gt(products.quantity, 0));
