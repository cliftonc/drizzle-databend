...
describe('aggregate functions', () => {
  ...
  describe('basic aggregates via ORM', () => {
    it('should compute count()', async () => {
      const result = await db.select({ cnt: count() }).from(products);
      expect(result).toHaveLength(1);
      expect(Number(result[0]!.cnt)).toBe(5);
    });

    it('should compute avg()', async () => {
      const result = await db.select({ avgPrice: avg(products.price) }).from(products);
      expect(result).toHaveLength(1);
      expect(Number(result[0]!.avgPrice)).toBeGreaterThan(0);
    });

    it('should compute sum()', async () => {
      const result = await db.select({ total: sum(products.quantity) }).from(products);
      expect(result).toHaveLength(1);
  ...
  describe('COALESCE wrapping', () => {
    ...
    it('should return 0 for COALESCE with impossible filter', async () => {
      const result = await db
        ...
        .from(products)
        .where(eq(products.category, 'nonexistent'));
      expect(result).toHaveLength(1);
  ...
  describe('statistical functions', () => {
    ...
    it('should check VAR_POP support', async () => {
      try {
        const result = await db
    ...
    it('should check VAR_SAMP support', async () => {
      try {
        const result = await db
