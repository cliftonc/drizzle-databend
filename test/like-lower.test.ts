...
describe('LIKE and LOWER patterns', () => {
  ...
  it('should handle basic LIKE', async () => {
    const result = await db.select().from(users).where(like(users.name, 'Ali%'));
    expect(result).toHaveLength(1);
    expect(result[0]!.name).toBe('Alice');
  });

  it('should handle LIKE with underscore wildcard', async () => {
    const result = await db.select().from(users).where(like(users.name, 'Bo_'));
    expect(result).toHaveLength(1);
    expect(result[0]!.name).toBe('Bob');
  });

  it('should handle LIKE with percent in middle', async () => {
    const result = await db.select().from(users).where(like(users.name, 'A%e'));
    expect(result).toHaveLength(1);
    expect(result[0]!.name).toBe('Alice');
  });
  ...
  it('should handle LOWER() in where clause via sql expression', async () => {
    const result = await db
      .select()
      .from(users)
      .where(eq(sql`LOWER(${users.name})`, 'alice'));
    expect(result).toHaveLength(1);
    expect(result[0]!.name).toBe('Alice');
  });
  ...
  it('should handle LOWER + LIKE via sql expression', async () => {
    ...
    const result = await db
      .select()
      .from(users)
      .where(sql`LOWER(${users.name}) LIKE ${pattern}`);
    expect(result).toHaveLength(1);
  });
  ...
  it('should handle case-insensitive search across categories', async () => {
    ...
    const result = await db
      .select()
      .from(products)
      .where(sql`LOWER(${products.category}) LIKE ${pattern}`);
